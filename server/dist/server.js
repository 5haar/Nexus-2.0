...
const analyzeScreenshot = async (filePath, mime, existingCategories) => {
    ...
    const response = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'You summarize screenshots and categorize them. Extract on-screen text accurately. Choose at most 2 categories total, preferring existing categories when they fit.',
            },
            {
                role: 'user',
                content: [
    ...
    try {
        ...
        if (isImage) {
            ...
            const pickedNew = (analysis.newCategories ?? [])
                .map((c) => canonicalizeCategory(c))
                .filter((c) => !!c && !existingSet.has(c));
            const categories = Array.from(new Set([...pickedExisting, ...pickedNew])).slice(0, Math.max(1, Math.min(10, MAX_CATEGORIES_PER_DOC)));
            doc = {
                id,
                userId,
                filePath: usablePath,
