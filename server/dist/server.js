...
const saveDocToDb = async (doc) => {
    ...
    const categories = Array.from(new Set((doc.categories ?? []).map((c) => canonicalizeCategory(c)).filter(Boolean))).slice(0, 5);
...
const analyzeScreenshot = async (filePath, mime) => {
    if (!openai)
        throw new Error('Missing OPENAI_API_KEY');
    ...
    const response = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'You summarize and categorize screenshots. Extract on-screen text accurately and return concise categories (3-5).',
            },
            {
                role: 'user',
                content: [
...
const buildRetrievalContext = async (query, topK, userId) => {
    ...
    const context = ranked
        .map(({ doc, score }) => `score:${score.toFixed(3)} | caption:${doc.caption} | categories:${doc.categories.join(', ')} | text:${doc.text}`)
        .join('\n');
    return { context, ranked };
};
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (isImage) {
            ...
            const id = randomId();
            const categories = Array.from(new Set((analysis.categories ?? []).map((c) => canonicalizeCategory(c)).filter(Boolean))).slice(0, 5);
            doc = {
                id,
                userId,
                filePath: usablePath,
    ...
    try {
        ...
        const response = await openaiClient.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a retrieval assistant. Use the provided screenshot context to answer briefly. If unsure, say so.',
                },
