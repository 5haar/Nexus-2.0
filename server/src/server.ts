...
const resolvePlanFromProductId = (productId: string): PlanId | null => {
  const trimmed = String(productId ?? '').trim();
  if (!trimmed) return null;
  if (trimmed === IAP_PRODUCT_IDS.starter || trimmed.endsWith('.starter') || trimmed === 'starter') return 'starter';
  if (trimmed === IAP_PRODUCT_IDS.pro || trimmed.endsWith('.pro') || trimmed === 'pro') return 'pro';
  if (trimmed === IAP_PRODUCT_IDS.max || trimmed.endsWith('.max') || trimmed === 'max') return 'max';
  return null;
};
...
const upsertEntitlement = async (entry: Entitlement) => {
  const now = Date.now();
...
const postAppleReceipt = async (url: string, receipt: string) => {
...
const verifyAppleReceipt = async (receipt: string) => {
  if (!APPLE_IAP_SHARED_SECRET) throw new Error('APPLE_IAP_SHARED_SECRET is not set');
...
const listSearchableDocs = async (userId: string) => {
  ...
  return db.docs.filter(
    (d) => d.userId === userId && d.mediaType === 'image' && d.embedding.length && (d.categories?.length ?? 0) > 0,
  );
};
...
const listUserCategories = async (userId: string) => {
  ...
  if (mysqlPool) {
    await ensureSchemaOnce();
    const [rows] = (await mysqlPool.execute(
      `SELECT c.name AS name, COUNT(d.id) AS cnt
       FROM categories c
...
const buildRetrievalContext = async (
  ...
  if (docId) {
    ...
    if (!doc) {
      return { context: '', ranked: [] as { doc: ScreenshotDoc; score: number }[], reason: 'Screenshot not found.' };
    }
...
app.post('/api/iap/verify', async (req, res) => {
  ...
  const receipt = String((req.body as any)?.receipt ?? '').trim();
  const requestedProductId = String((req.body as any)?.productId ?? '').trim();
  if (!receipt) return res.status(400).json({ error: 'Receipt is required' });
  if (!APPLE_IAP_SHARED_SECRET) return res.status(400).json({ error: 'IAP secret is not configured' });
  try {
    ...
    const transactions = rawTransactions.filter((tx: any) => {
      if (!requestedProductId) return true;
